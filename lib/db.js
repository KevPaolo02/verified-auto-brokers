...
// Insert a public-submitted broker report into the internal review queue.
// Default severity is 'medium' — admins promote/demote when triaging via SQL.
export async function createInternalFlag({
  mc,
  dot = null,
  reason,
  severity = "medium",
  ...
  if (!sql) throw new Error("DB not configured");
  await sql`
    INSERT INTO internal_flags (mc, dot, reason, severity, source, created_by)
    VALUES (${mc}, ${dot}, ${reason}, ${severity}, ${source}, ${createdBy})
  `;
  return { ok: true };
}
