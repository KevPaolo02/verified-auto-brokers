...
const UnlockFlow = ({ broker, onClose, onSubmit }) => {
  const [data, setData] = lS({ name:"", email:"", phone:"", reason:"shipping" });
...
const MatchFlow = ({ onClose, onSubmit }) => {
  const [step, setStep] = lS(1);
  const [data, setData] = lS({ what:"sedan", priority:"price", transport:"open", route:"long", contact:"", phone:"", email:"" });
  const set = (k,v) => setData(d => ({...d, [k]: v}));
  const submit = () => { onSubmit && onSubmit({kind:"match", data}); setStep(99); };

  const matched = data.priority === "trust"
    ? window.FMCSA_BROKERS.filter(b => !b.flagged).sort((a,b) => b.rating - a.rating).slice(0, 3)
    : data.transport === "enclosed"
    ? window.FMCSA_BROKERS.filter(b => !b.flagged && b.specialties.some(s => /enclosed/i.test(s))).slice(0, 3)
    : window.FMCSA_BROKERS.filter(b => !b.flagged).sort((a,b) => b.fleet_partners - a.fleet_partners).slice(0, 3);
        ...
        <Section title="Your 3 matches">
          {matched.map((b, i) => (
            <div key={b.mc} style={{
              ...
              <div>
                <div style={{fontFamily:"'Inter Tight'", fontSize: 16, fontWeight: 600}}>{b.name}</div>
                <UI.Mono style={{fontSize:10.5, color:"var(--muted)", letterSpacing:"0.1em", textTransform:"uppercase"}}>
                  {b.mc} · {b.years}Y · ★ {b.rating} ({b.reviews.toLocaleString()})
                </UI.Mono>
              </div>
              <UI.Stamp tone="verified" small>MATCH</UI.Stamp>
            </div>
          ))}
          <p style={{fontFamily:"'Inter Tight'", fontSize: 14, lineHeight: 1.5, color:"var(--muted)", marginTop: 14}}>
            Each broker has been notified about your shipment. Expect calls within the next hour. We've also emailed you a contact card for each.
          </p>
        </Section>
      )}
...
const AlertsPage = ({ onPick }) => (
  <section style={{background: "var(--paper)", padding: "40px 24px 80px"}}>
    <div style={{maxWidth: 1280, margin: "0 auto"}}>
      <UI.Eyebrow color="var(--red)">⚠ Public Notice · Updated Hourly</UI.Eyebrow>
      <h1 style={{fontFamily:"'Instrument Serif'", fontSize: "clamp(56px, 7vw, 96px)", lineHeight:0.95, margin:"12px 0 0", letterSpacing:"-0.02em", fontWeight:400}}>
        Brokers <em style={{color:"var(--red)"}}>flagged</em> in the last 30 days.
      </h1>
      <p style={{fontFamily:"'Inter Tight'", fontSize: 17, lineHeight: 1.5, color:"var(--ink)", maxWidth: 640, marginTop: 18}}>
        These brokers have either lost their FMCSA authority, had their bond claimed, or accumulated a complaint pattern that warrants caution. We do not accept advertising from any broker on this list.
      </p>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span>BLACKLIST · 5 RECENT</span>
          <span>FILED · LAST 30 DAYS</span>
        </div>
        {window.SCAM_ALERTS.map((a, i) => (
          <div key={i} style={{
            display:"grid", gridTemplateColumns: "150px 1fr 100px",
            padding: "20px 22px", gap: 18, alignItems:"start",
            borderBottom: i < window.SCAM_ALERTS.length - 1 ? "1px solid var(--rule)" : "none",
            background: "var(--red-tint)"
          }}>
            <UI.Mono style={{fontSize: 12, color:"var(--ink)"}}>{a.mc}</UI.Mono>
            <div>
              <div style={{fontFamily:"'Inter Tight'", fontSize: 18, fontWeight: 600}}>{a.name}</div>
              <div style={{fontFamily:"'Inter Tight'", fontSize: 14, color:"var(--ink)", marginTop: 4}}>{a.reason}</div>
            </div>
            <div style={{textAlign:"right"}}>
              <UI.Mono style={{fontSize:11, color:"var(--muted)"}}>{a.date}</UI.Mono>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <div>
          <UI.Eyebrow>Got burned by a broker?</UI.Eyebrow>
          ...
          <p style={{fontFamily:"'Inter Tight'", fontSize: 14, lineHeight: 1.5, color:"var(--ink)", marginTop: 8, maxWidth: 520}}>
            Three or more verified reports trigger a manual review. We cross-check against FMCSA filings and BBB complaints before flagging publicly.
          </p>
        </div>
  ...
  <footer style={{background: "var(--ink)", color: "var(--paper)", padding: "56px 24px 32px", marginTop: 0}}>
    <div style={{maxWidth: 1280, margin:"0 auto"}}>
      ...
      <div style={{marginTop: 56, paddingTop: 24, borderTop: "1px solid rgba(244,241,234,0.18)", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
        ...
        <UI.Mono style={{fontSize: 10.5, letterSpacing:"0.14em", textTransform:"uppercase", opacity: 0.6}}>
          DATA SOURCES · FMCSA · BBB · DIRECT FILINGS
        </UI.Mono>
      </div>
    </div>
  </footer>
...
window.LEAD = { QuoteFlow, UnlockFlow, MatchFlow, AlertsPage, Footer, Modal };
