...
const TopBar = ({ stats }) => {
  ...
  const lastImported = stats?.last_imported_at
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex", gap: 24, textTransform: "uppercase"}}>
        <span>FMCSA PUBLIC-RECORD LOOKUP · {total} BROKERS TRACKED</span>
        <span style={{opacity:.6}}>REGISTRY UPDATED {lastImported}</span>
      </div>
...
const Ticker = ({ stats }) => {
  // Real metrics only — pulled from getRegistryStats(). When stats are loading
  // we render a single neutral placeholder rather than fabricating numbers.
  if (!stats) return null;
  const items = [
    { num: Number(stats.total_brokers || 0).toLocaleString(), label: "Licensed Brokers Tracked" },
    { num: Number(stats.active_authority || 0).toLocaleString(), label: "Active Authority" },
    { num: Number(stats.with_bond_on_file || 0).toLocaleString(), label: "Bond on File" },
    { num: Number(stats.claimed_verified || 0).toLocaleString(), label: "Claimed & Verified" },
    { num: Number(stats.claimed_pending || 0).toLocaleString(), label: "Claims Pending Review" },
  ];
  return (
    <div style={{
